...
resource "google_compute_instance" "default" {
  ...
  provisioner "local-exec" {
    command = <<EOH
curl -H 'Content-Type: application/json' -c cookies.txt -b cookies.txt -k https://${google_compute_instance.default.network_interface.0.access_config.0.nat_ip}/api/v0/logi-d '{"username":"admin","password":"${var.sshpassword}"}' \
